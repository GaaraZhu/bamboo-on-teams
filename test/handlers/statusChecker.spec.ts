  ...
  describe("getJobPageUrl", () => {
    const testCases = [
      ...
      },
      {
        ...
        output: "https://test.co.nz/deploy/viewDeploymentResult.action?deploymentResultId=1243",
      },
      {
        input: {
            resultKey: " ",
    ...
    testCases.forEach((testCase) => {
      it("Input: " + JSON.stringify(testCase.input), async () => {
        try {
          expect(getJobPageUrl(testCase.input.resultKey, testCase.input.isBuild)).toEqual(testCase.output);
        } catch (err: any) {
